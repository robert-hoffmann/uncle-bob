# Tailwind Legacy-to-Modern Migration

> Verify migration paths against the latest official Tailwind CSS documentation via web search.

## Purpose

Capture high-impact legacy-to-modern Tailwind differences so generated guidance consistently produces modern, upgrade-safe output.

## Migration Matrix (Legacy Pattern → Modern Pattern → Why It Matters → Canonical Replacement)

| Legacy pattern | Modern pattern | Why it matters | Canonical replacement |
| --- | --- | --- | --- |
| `@tailwind base; @tailwind components; @tailwind utilities;` | `@import "tailwindcss";` | Modern Tailwind is CSS-first and no longer uses directive triplets as the default entrypoint. | Replace old directives with a single `@import "tailwindcss";` in the main stylesheet. |
| PostCSS plugin `tailwindcss` | PostCSS plugin package `@tailwindcss/postcss` | Modern Tailwind moved PostCSS integration to a dedicated package. | Install/use `@tailwindcss/postcss` in PostCSS config when PostCSS is required. |
| CLI via `tailwindcss` package/binary | CLI via `@tailwindcss/cli` | Modern Tailwind moved CLI into a dedicated package. | Use `npx @tailwindcss/cli ...` for CLI workflows. |
| Prefix in class token (`tw-bg-red-500`) | Prefix as variant (`tw:bg-red-500`) | Class name generation/consumption changes in modern Tailwind. | Migrate prefixed classes to `tw:*` variant style. |
...
| Safelist-only workflow in JS config | `@source inline()` in CSS when explicit include is needed | Modern CSS-first source controls replace many safelist-only use cases. | Add `@source inline()` for explicit generation targets. |
...
- Do not assume every legacy JavaScript config behavior maps 1:1 in modern Tailwind.
